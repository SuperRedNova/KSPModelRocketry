...
namespace KSPModelRocketry
{
    public class VariableTexture : PartModule
    {
        ...
        UI_ChooseOption(options = new string[] { "Plain", "Quarter 1", "Quarter 2",
        ...
        public void Update()
        {
            ...
            if (HighLogic.LoadedSceneIsEditor && ((
                ...
                opacity != part.FindModelComponent<Renderer>(textureMeshName).material.GetFloat("_Opacity"))))
            {
                setPattern();
                setColor();
            }
        ...
        /// <summary>
        /// Changes the color of a mesh of a part implementing this PartModule based on it's red, green, and blue values.
        /// </summary>
        public void setColor()
        {
            if (texRend == null)
            {
                ...
                {
                    mat = new Material(LoadShader.MRDiffuse);
                }
            }
            Debug.Log("[ModelRocketry] opacity: " + texRend.material.GetFloat("_Opacity"));
            if (opacity != texRend.material.GetFloat("_Opacity"))
            {
                if (texRend.material.GetFloat("_Opacity") == 1)
                {
                    if(mat.shader != LoadShader.MRDiffuse) mat.shader = LoadShader.MRDiffuse;
                }
                else
                {
                    if(mat.shader != LoadShader.MRDiffuseAlpha) mat.shader = LoadShader.MRDiffuseAlpha;
                }
            }
            opacity = texRend.material.GetFloat("_Opacity");
            if (switchColor)
            {
                color1.r = red;
                color1.g = green;
            ...
            tex.Apply();
            if (texpat == "Checker") tex = LoadShader.Checker;
            tex.filterMode = FilterMode.Point;
            mat.mainTexture = tex;
            mat.SetColor("_Color", color1);
            mat.SetColor("_Color2", color2);
            mat.mainTextureScale = new Vector2(texRep, 1);
